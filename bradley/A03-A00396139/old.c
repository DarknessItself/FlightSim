...
GLint loadModel(GLint id, GLint numPoints, char* fileName, int colorScheme);
...
GLint mountainDetailAccuracy = 81;
...
GLint loadModel(GLint id, GLint numPoints, char* fileName, int colorScheme) {
	...
	while (fgets(line, 256, file) != NULL) {
		...
		int found = sscanf_s(line, "v %f %f %f", &x, &y, &z);
		...
		else if ((found = sscanf_s(line, "n %f %f %f", &nx, &ny, &nz)) != 0) {
		...
		else if ((found = sscanf_s(line, "g %s", objectName)) != 0) {
			//printf("%d %s\n", found, objectName);
			if (colorScheme == 1) {
				if (objectCount <= 3) {
		...
		else if ((found = sscanf_s(line, "%c ", &ch)) != 0 && ch == 'f') {
			int f;
			//printf("char %c\n", ch);
			char *token;
			token = strtok_s(line, " ");
...
*/
GLint loadGLTexture(const char * filename, int wrap, int width, int height) {
	GLint texture;
	BYTE * data;
	FILE * file;
